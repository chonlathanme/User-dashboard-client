  ...
  useEffect(() => {
    if (searchQuery) {
      const filtered = user
        .slice(1)
        .filter((row) =>
          row[1]?.toString().toLowerCase().includes(searchQuery.toLowerCase())
        );
      setFilteredUsers(filtered);
    } else {
      setFilteredUsers(user.slice(1));
    }
    setCurrentPage(1);
  }, [searchQuery, user]);
            ...
            <div className="overflow-y-auto max-h-[600px]">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    {['ID', 'Firstname', 'Lastname', 'Email', 'Gender', 'City', 'Country', 'Country Code', 'State', 'Street Address', 'Job title', 'Company', 'Photo'].map((header, index) => (
                      <th
                        key={index}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {Array.isArray(usersToDisplay) && usersToDisplay.map((row, rowIndex) => (
                    <tr
                      key={rowIndex}
